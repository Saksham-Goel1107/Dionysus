...
export const loadGithubRepo = async (githubUrl: string, githubToken?: string) => {
  try {
    ...
    throw new Error(
      `Unable to fetch repository files: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
};
...
export const indexGithubRepo = async (
  ...
  try {
    ...
    if (docs.length > MAX_FILES) {
      console.log(
        `Processing only first ${MAX_FILES} files out of ${docs.length} to avoid rate limits`,
      );
    }
  ...
  try {
    ...
    try {
      ...
      if (url.host !== 'github.com') {
        throw new Error(
          "Invalid GitHub URL host. Please provide a URL with the host 'github.com'.",
        );
      }
    ...
    if (!githubOwner || !githubRepo) {
      ...
      throw new Error(
        "Could not extract owner and repository name from the GitHub URL. Please provide a valid GitHub repository URL in the format 'owner/repo' or 'github.com/owner/repo'.",
      );
    }
    ...
    try {
      ...
      if (error.status === 404) {
        throw new Error(
          `Repository ${githubOwner}/${githubRepo} not found. Please check the URL or provide a personal access token if it's a private repository.`,
        );
      } else if (error.status === 401 || error.status === 403) {
        throw new Error(
          `Access denied to repository ${githubOwner}/${githubRepo}. Please provide a valid personal access token with repo scope.`,
        );
      } else {
