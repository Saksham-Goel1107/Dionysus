...
export default function SurveyPage() {
    ...
    if (currentFeatures.includes(feature)) {
      ...
      setValue(
        ...
        updatedFeatures.length > 0
          ? (updatedFeatures as [string, ...string[]])
          : [],
        { shouldValidate: true }
      );
    } else {
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50'} py-12 px-4 sm:px-6 lg:px-8`}>
      <div className="max-w-3xl mx-auto">
        <div className={`shadow-lg overflow-hidden rounded-xl ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'}`}> 
          <div className={`px-4 py-5 sm:px-6 border-b ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'} flex items-center justify-between`}>
            <div>
              <div className="flex gap-4 items-center">
              <UserButton/>
              <h1 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Welcome to Dionysus!</h1>
              </div>
              <p className={`mt-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>Please complete this quick survey to help us provide you with the best experience. This information helps us tailor our service to your needs.</p>
            </div>
            <button
              type="button"
          ...
          <form ref={formRef} onSubmit={handleSubmit(onSubmit)} className={`px-4 py-5 sm:p-6 space-y-8 ${theme === 'dark' ? 'bg-slate-800' : ''}`}> 
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="companyName" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
                ...
                <div>
                  <label htmlFor="companySize" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
                    Company Size*
                  </label>
                  <select
                    id="companySize"
              ...
              <div>
                <label htmlFor="industry" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
                  Industry*
                </label>
                <select
                  id="industry"
              ...
              <div>
                <label htmlFor="role" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
                  Your Role*
                </label>
                <select
                  id="role"
            ...
            <div className="space-y-6 pt-4 border-t border-gray-200 dark:border-gray-700">
              <h2 className={`text-xl font-semibold pt-4 ${theme === 'dark' ? 'text-slate-100' : 'text-slate-900'}`}> 
                Usage & Discovery
              </h2>
              
              <div>
                <label htmlFor="usagePurpose" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
              ...
              <div>
                <label htmlFor="hearAboutUs" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
            ...
            <div className="space-y-6 pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <div>
                <span className={`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
              ...
              <div>
                <label htmlFor="developmentExperience" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
                ...
                {errors.developmentExperience && (
                  <p className="mt-1 text-sm text-red-600">{errors.developmentExperience.message}</p>
                )}
              </div>

              <div>
                <label htmlFor="githubExperience" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
            ...
            <div className="space-y-6 pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <div>
                <label htmlFor="feedbackFrequency" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
              ...
              <div>
                <label htmlFor="additionalFeedback" className={`block text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}> 
