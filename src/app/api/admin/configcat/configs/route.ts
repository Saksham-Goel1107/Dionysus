...
/**
 * Admin API: Get configs for a product
 * GET /api/admin/configcat/configs?productId=xxx
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const configs = await client.getConfigs(productId);

    return NextResponse.json({ configs });
  } catch (error) {
    console.error('Error fetching ConfigCat configs:', error);
    return NextResponse.json(
