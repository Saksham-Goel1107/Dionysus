...
const Advanced = () => {
  ...
  // List of blocked domains (official, famous, sensitive)
  ...
  function isBlockedUrl(url: string) {
    try {
      const u = new URL(url);
      // Only block exact matches or subdomains of the listed domains, not all vercel.app deployments
      return BLOCKED_DOMAINS.some(domain => {
        if (domain === "vercel.app") {
          // Only block vercel.app root, not custom deployments
  ...
  const handleStressTest = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/stress-test", {
        ...
        body: JSON.stringify({ url, users, simulateReal }),
      });
    ...
    <Protect
      ...
      fallback={
        <div className="flex min-h-[60vh] flex-col items-center justify-center space-y-4">
          ...
          <p className={`text-center max-w-md ${resolvedTheme === "dark" ? "text-gray-200" : "text-gray-600"}`}>
            Access to Advanced Tab is available exclusively for <span className="font-semibold text-yellow-700">Dionysus Advance Pack</span> subscribers. Upgrade your plan to unlock this feature.
          </p>
          <Link href="/subscriptions">
            <Button className="bg-yellow-600 text-white hover:bg-yellow-700" size="lg">
              Upgrade Now
            </Button>
          </Link>
        </div>
