...
const CodeAnalytics = () => {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/code-analytics?repoUrl=${encodeURIComponent(project.githubUrl)}`);
  ...
  const handleSendChat = async () => {
    ...
    const newHistory = [
      ...chatHistory,
      { role: 'user' as 'user', content: chatInput }
    ];
  ...
  const avgComplexity = analytics && analytics.length ? (analytics.reduce((sum: number, a: any) => sum + (a.aggregate?.cyclomatic || 0), 0) / analytics.length).toFixed(2) : 0;
...
const totalFunctions = analytics ? (analytics as FileAnalytics[]).reduce((sum, a) => sum + (a.functions?.length || 0), 0) : 0;
  // Enhanced: sort by complexity, highlight top files
  const topComplexFiles = analytics ? [...analytics].sort((a, b) => (b.aggregate?.cyclomatic || 0) - (a.aggregate?.cyclomatic || 0)).slice(0, 3) : [];
  ...
  return (
    <>
      <div className="w-full max-w-2xl mx-auto my-8 p-6 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md">
        <h2 className="text-2xl font-bold mb-4 flex items-center gap-2"><FaChartBar className="text-blue-600" /> Code Analytics</h2>
        <Button onClick={() => setShowModal(true)} className="bg-blue-600 text-white mb-2">Show Analytics</Button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 overflow-auto">
          <div className="bg-white dark:bg-gray-900 rounded-xl md:rounded-xl shadow-xl p-2 sm:p-6 w-full max-w-full sm:max-w-4xl relative max-h-[98vh] overflow-y-auto overflow-x-hidden mx-1 sm:mx-0" style={{width: '98vw'}}>
            ...
            <div className="flex flex-wrap gap-2 sm:gap-4 mb-4 justify-center w-full overflow-x-auto">
              <button onClick={() => setActiveTab('charts')} className={`px-2 sm:px-4 py-2 rounded flex items-center gap-2 text-xs sm:text-base min-w-[90px] ${activeTab==='charts'?'bg-blue-600 text-white':'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-200'}` }><FaChartBar /> Charts</button>
              <button onClick={() => setActiveTab('summary')} className={`px-2 sm:px-4 py-2 rounded flex items-center gap-2 text-xs sm:text-base min-w-[90px] ${activeTab==='summary'?'bg-blue-600 text-white':'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-200'}` }><FaListAlt /> Summary</button>
              <button onClick={handleShowAI} className={`px-2 sm:px-4 py-2 rounded flex items-center gap-2 text-xs sm:text-base min-w-[90px] ${activeTab==='ai'?'bg-blue-600 text-white':'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-200'}` }><FaRobot /> AI Explanation</button>
            </div>
            {activeTab === 'charts' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6 w-full overflow-x-hidden">
                <div className="w-full max-w-full flex justify-center items-center"><BarChart analytics={analytics} /></div>
                <div className="w-full max-w-full flex justify-center items-center"><PieChart analytics={analytics} /></div>
                <div className="w-full max-w-full flex justify-center items-center"><FunctionBarChart analytics={analytics} /></div>
                <div className="w-full max-w-full flex justify-center items-center"><QualityBarChart quality={quality} /></div>
                  ...
                  <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                    {topComplexFiles.map((f, i) => (
                      <li key={f.path} className="py-2 flex justify-between items-center">
                        ...
                        <span className="bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 px-2 py-1 rounded text-xs font-bold">Cyclomatic: {f.aggregate?.cyclomatic}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            {activeTab === 'summary' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                ...
                <div className="bg-green-100 dark:bg-green-900 rounded-lg p-4">
                  ...
                  <div className="text-sm text-green-800 dark:text-green-200">Avg. Cyclomatic Complexity</div>
                </div>
                <div className="bg-yellow-100 dark:bg-yellow-900 rounded-lg p-4">
                  <div className="text-3xl font-bold">{totalFunctions}</div>
                  <div className="text-sm text-yellow-800 dark:text-yellow-200">Total Functions</div>
                </div>
              </div>
            )}
            {/* Quality Analysis */}
            {activeTab === 'summary' && (
              <div className="mt-6">
                ...
                {quality.length === 0 && !qualityLoading && <p className="text-green-600">No major issues found.</p>}
                {quality.length > 0 && (
                  <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                    {quality.map((q, i) => (
                      <li key={q.path} className="py-2 flex flex-col md:flex-row md:justify-between md:items-center">
                        <span className="font-mono text-xs md:text-sm">{q.path}</span>
                        <span className="text-xs text-red-600 dark:text-red-300 mt-1 md:mt-0">{q.issues.join(", ")}</span>
                      </li>
                    ))}
                  </ul>
                )}
            ...
            {activeTab === 'ai' && (
              <div className="mt-4 min-h-[120px]">
                <div className="border rounded-lg p-2 sm:p-4 max-h-80 overflow-y-auto mb-2 bg-white dark:bg-[#18181b] border-gray-200 dark:border-gray-700 transition-colors duration-200 w-full" style={{minHeight: 180}}>
                  {chatHistory.length === 0 && <p className="text-gray-400">Ask anything about your codebase analytics!</p>}
                  {chatHistory.map((msg, i) => (
                    <div key={i} className={`mb-2 flex ${msg.role==='user'?'justify-end':'justify-start'}`}>
                      <div className={`rounded-lg px-2 py-2 sm:px-3 sm:py-2 max-w-[90vw] sm:max-w-[80%] text-sm break-words ${msg.role==='user'?'bg-blue-600 text-white':'bg-gray-50 dark:bg-[#23272e] text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700 transition-colors duration-200'}`}>
                        <MDEditor.Markdown source={msg.content} className="prose dark:prose-invert max-w-none" />
                      </div>
                    </div>
                  ))}
                ...
                <form className="flex flex-col sm:flex-row gap-2 w-full" onSubmit={e => {e.preventDefault(); handleSendChat();}}>
                  <input
                    type="text"
                    ...
                    placeholder={analytics ? "Ask a question about your codebase..." : "No analytics available. Please connect your repo."}
                    value={chatInput}
                    ...
                    style={{overflow: 'auto'}}
                  />
                  <Button type="submit" className="bg-blue-600 text-white px-4 w-full sm:w-auto" disabled={chatLoading || !chatInput.trim() || !analytics}>Send</Button>
                </form>
                <div className="text-xs text-gray-400 mt-1">AI answers are powered by Gemini and your code analytics.</div>
              </div>
            )}
            <div className="mt-6">
